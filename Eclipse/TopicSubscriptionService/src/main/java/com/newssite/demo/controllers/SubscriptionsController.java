...
public class SubscriptionsController {
	...
	private Map<String, TopicSubscription> activeTopics = new HashMap<String, TopicSubscription>();
	private Map<String, ClientSession> activeSessions = new HashMap<String, ClientSession>();

	// Methods
	public NewsTopicFilters getTopicFiltersFromURI(String uri) {
		//Get all of the query parameters provided from the String uri
		...
		// Return a new NewsTopicFilters object using the builder pattern
	...
	public void newsWasRequested(String stompDestination) {
		...
		// Check if the current microservice has the same STOMP subscription that was
		// recently updated and is in use
		if (topicSubscription != null && topicSubscription.getSubscriptions() > 0) {
			...
			// Get a list of all registered microservices with the eureka server (including
			// this instance)
	...
	public void onApplicationEvent(SessionSubscribeEvent event) {
		...
		if (nativeHeaders != null) {
			...
			if (subscriptionDestination != null && !subscriptionDestination.contains("/user")) {
				...
				String kafkaTopic = DigestUtils.sha1Hex(topicName + filters.toString());
				...
				if (!isSubscribed) {
					...
					if (topicEntry != null) {
						...
						KafkaConsumerUtil.createTemporaryConsumer(kafkaTopic,
								new TopicAckMessageListener(new NewsRefresherTopicProcessor(subscriptionDestination,
										sessionID, simpMessengerTemplate), false),
								KafkaConfig.getArticleConsumerProps(), 10000);
					} else {

						if (KafkaConsumerUtil.articleTopicExists(kafkaTopic)) {

							activeTopics.put(subscriptionDestination, new TopicSubscription(topicName, kafkaTopic,
									subscriptionDestination, filters, kafkaEventTemplate));
							...
							KafkaConsumerUtil.startOrCreateConsumers(kafkaTopic,
									new TopicAckMessageListener(
											new NewsTopicProcessor(subscriptionDestination, simpMessengerTemplate),
											true),
									1, KafkaConfig.getArticleConsumerProps());
